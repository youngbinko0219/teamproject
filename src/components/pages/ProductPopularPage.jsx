...
import popularp from "../../assets/images/productlist/popularp.jpg";
...
const ProductPopularPage = () => {
  ...
  useEffect(() => {
    const fetchPopularProducts = async () => {
      setLoading(true);
      try {
        const response = await axios.get("http://localhost:8080/products/best");
        const formattedProducts = response.data.data.slice(0, 12).map((product) => ({
    ...
    <>
      ...
      <div className="product-list-container">
        ...
        <main className="center-section">
          <img className="category-title-detail" src={popularp} />
